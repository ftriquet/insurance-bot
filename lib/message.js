...
const handleUsecase = (oauth, result, userId) => {
  return usecase.getReply(oauth, result.entities, conversations, userId)
}
...
const replyText = (text, userId) => {
  ...
  return new Promise((success, failure) => {
    Promise.all([salesforce.login(), request.analyseText(text)])
      .then(([oauth, result]) => {
        ...
        if (intent && conversations[userId]) {
          ...
          if (intent.slug === usecase.handledIntent) {
            return success(handleUsecase(oauth, result, userId))
...
module.exports = { replyText, replyMessage, consumeDb }
